...
- [x] เตรียม T-730 Passive Gate Attribution เป็น Debug external collector บน BASE result: ให้ completed peaks มี window ordinal ร่วมกัน, แสดง blocker set จาก exact-paired retained evidence สำหรับ `SYNC/BIL-L/BIL-R/RATIO/STD-H/RES-A/RES-H`, `UNATTRIBUTED`, overflow และ measurement invalid guard โดยไม่แก้ `BasicBounceDetector`, threshold, Counter, Result หรือ History; Pure-Kotlin regression ผ่าน 64/64
- [ ] รัน T-730 smoke 3 jumps และ formal 20–30 jumps บน Samsung Galaxy S23 Ultra ด้วย Music OFF/แสงปกติ; ต้องไม่มี motion ระหว่าง countdown และหลังเห็น `ACTIVE P0` ให้ยืนนิ่งประมาณ 1 วินาทีก่อนเริ่ม; ห้ามมี rope trip, auto-pause หรือ `INVALID-RESTART` และใช้ผลได้เฉพาะ `U0/OV0` พร้อมเทียบ peak ordinal กับวิดีโอ
