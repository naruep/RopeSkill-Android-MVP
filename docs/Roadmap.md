...
- [ ] รัน Android unit tests/Build และ T-730 V13 smoke บน Samsung Galaxy S23 Ultra เพื่อยืนยัน timestamps, accepted anchors, region totals, explicit invalid reason, trace/display capacity, BASE isolation, performance/stability และ Result/History
- [ ] หลัง V13 smoke ผ่าน ให้รัน formal ด้วย Music OFF/แสงปกติ: counted opening bookend 1 ครั้ง + target Basic Bounce 20 ครั้ง + counted closing bookend 1 ครั้ง รวม Actual 22; ใช้วิดีโอยืนยันขอบเขตและวิเคราะห์ gate เฉพาะ `WINDOW` โดยยกเลิกรอบเมื่อมี rope trip, auto-pause ก่อน seal/ระหว่าง measurement, invalid reason หรือ trace overflow; ใช้ Screen Recorder จับ `SEALED POST-EXIT` เพราะ auto-pause ที่ตามมาอาจล้าง overlay
