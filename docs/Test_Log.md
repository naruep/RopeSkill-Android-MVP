...
| T-711 | Rejected Foot and Cooldown Evidence V7 | รัน unit tests; Basic Bounce 10 และ heel raise 20 โดยให้รองเท้าอยู่ในเฟรม แล้วอ่าน `REJECTED TAKEOFF V7` และ `COOLDOWN V7` | rejected cycles แสดง heel/toe หรือ `FOOT N/A`; แสดงจำนวน landing ที่ถูก cooldown ระงับและ interval เทียบ `250ms`; Counter, threshold และ state machine ไม่เปลี่ยน | เพิ่ม diagnostic แบบ passive เพื่อแยก ankle-threshold miss จาก cooldown suppression; รอ unit tests และอุปกรณ์จริง | Not Run |
