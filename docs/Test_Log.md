...
| T-211 | Branded system splash | รัน unit tests/Build; force-stop แล้วเปิดจาก Launcher; เปิดซ้ำแบบ warm start; ทดสอบ Dark, Light และ System theme; ตรวจ Launcher icon, splash และ Home | ไม่มีพื้นขาว; splash พื้น `#071426`; คนและเชือกเห็นครบโดยไม่มี adaptive-icon frame ซ้อน; ไม่มี delay เพิ่ม; Home/Launcher icon เดิมและทุก theme เปิดถูกต้อง | ผู้ใช้ยืนยันว่าทุกอย่างถูกต้อง ยกเว้นยอดวงเชือก; ภาพ `Screenshot 2026-07-26 124639.png` ของรอบ 128dp แสดงวงเชือกเกือบครบ แต่การวัดพบว่ายอดที่คาดไว้ `y=26.9px` เริ่มปรากฏจริงที่ `y=28px` จึงยังถูก mask ราว 1px ลดเป็น 124dp เพื่อเพิ่ม margin ประมาณ 3px และรอทดสอบซ้ำ | Partial Pass |
