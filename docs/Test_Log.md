...
| T-606 | Camera permission overlay | เปิด Training ขณะที่ยังไม่อนุญาต Camera permission แล้วอนุญาตจากปุ่ม `ALLOW CAMERA`; ปิดและเปิดแอปแล้วทดสอบซ้ำ | ก่อนอนุญาตเห็นเฉพาะข้อความและปุ่ม permission โดยไม่มี Positioning/Tracking/Detector/diagnostic บัง; หลังอนุญาต overlay กลับมาและกล้องทำงาน | รอบแรกผ่าน แต่หลังปิดและเปิดแอป `SHOW FULL BODY` กับ status labels กลับมาบัง permission; เปลี่ยนให้ Training ใช้ permission state ชุดเดียวและตรวจใหม่เมื่อ Resume รอทดสอบซ้ำ | Retest |
| T-607 | Training action colors | ตรวจ `PAUSE`, `RESUME` และ `FINISH` ระหว่าง Training ใน Dark/Light | Pause/Resume ใช้สี primary; Finish ใช้กรอบและข้อความสี error; ข้อความอ่านชัด สถานะ disabled ถูกต้อง และการกดทำงานเหมือนเดิม | รอ Build และทดสอบบนอุปกรณ์จริง | Not Run |
