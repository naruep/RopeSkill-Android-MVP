...
| KI-010 | 2026-07-24 | กระโดดเชือกจริงที่ประมาณ 133 jumps/min นับได้ 11/20 แม้ไม่มี false count หลังหยุด | High | Investigating | `AIR 11 / LAND 11` ยืนยันว่า accepted Takeoff ลงครบ; `ANK 11` เด่นกว่า HIP/SYNC และ accepted ankle-rise อยู่ใกล้ threshold 0.045 แต่ยังไม่มีค่าของรอบที่ถูกปฏิเสธ จึงยังสรุป threshold ใหม่ไม่ได้ | เพิ่ม Debug-only `REJECTED TAKEOFF V5` เก็บ 3 rejected cycles ที่ ankle-rise สูงสุดโดยไม่เปลี่ยนการนับ แล้วใช้ T-706 ตัดสินจากข้อมูลจริงก่อนปรับ threshold |
