...
| KI-020 | 2026-07-29 | T-726 Continuous 100 Jumps รอบ 1 ในแสงปกตินับได้ 85/100 หรือ 85% ต่ำกว่าเป้าหมาย 95%; ผู้ใช้สะดุดเชือก 1 ครั้ง | High | Investigating | ไม่ใช่ Landing, cooldown หรือ performance regression เพราะ `AIR/LAND 85/85`, `SUP 0`, FPS 29.9, LAT 26/75ms และ SKIP ~0; trace ระหว่างกระโดดมี rejected cycles ต่อเนื่อง `A0.018/0.011/0.013/0.017` กับ `H0.120–0.161` และภายหลัง `A0.015 H0.138` โดยเหตุผล `ANK` จึงมีสมมติฐานหลักว่า smoothed ankle rise ของ jump จริงบางครั้งต่ำกว่า rescue floor `0.020`; ค่า `ANK 88` เป็นจำนวน diagnostic transitions ไม่ใช่จำนวน miss | รัน T-727 แบบ 100 jumps ต่อเนื่องภายใต้ baseline/เงื่อนไขเดิมหลังพัก และไม่นับรอบที่สะดุดเชือก; ยังไม่เปลี่ยน detector/threshold จนกว่าจะยืนยัน repeatability แล้วจึงออกแบบ experiment พร้อม heel raise, knee lift และ standing controls |
