...
| KI-022 | 2026-07-31 | Basic Bounce ภายใต้เงื่อนไขเดิมแปรผันจาก `21/22` เป็น `15/22`, `18/22` และ T-741 cadence-controlled `18/22`; changed-clothing ได้ `18/22` | High | Confirmed | Repeatability variance ยืนยันจาก T-741 แต่ causal gate/state ยังไม่ยืนยัน; visible diagnostics มี `ANKLE RISE TOO SMALL`, `FEET NOT SYNCHRONIZED` และบาง T-740 session มี `HIP RISE TOO SMALL`, ขณะที่ accepted `T/L`, `SUP`, performance และ stability ปกติ | ทำ T-742 existing-video missed-cycle audit เพื่อ map แต่ละ physical cycle กับ Counter/state/rejection evidence; ห้ามแก้ threshold หรือ detector จนกว่าจะมี cycle-level evidence หรือระบุ passive diagnostic gap ที่ชัดเจน |
