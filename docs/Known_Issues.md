...
| KI-020 | 2026-07-29 | Continuous 100 Jumps ในแสงปกตินับได้ T-726 85/100 และ T-727 93/100 ต่ำกว่าเป้าหมาย 95%; T-727 ไม่สะดุดเชือก | High | Confirmed / Investigating root cause | ทั้งสองรอบไม่ใช่ Landing, cooldown หรือ performance regression: T-727 ได้ `AIR/LAND 93/93`, `SUP 0`, FPS 30.0, LAT 25/46ms, IN/OUT 2389/2389 และ SKIP ~0; retained rejected peaks ที่แรงที่สุดคือ `A0.021/0.023 H0.133/0.128 ... ANK` และ `A0.020/0.022 H0.116/0.116 ... ANK`; peak แรกอยู่เหนือ average rescue floor `0.020` และ hip gate `0.100` แต่ยังถูกปฏิเสธ ภายใต้ decision logic ปัจจุบันจึงชี้ไปที่ per-ankle bilateral floor เป็นสมมติฐานหลัก อย่างไรก็ดี V10 ยังไม่เก็บค่า left/right ของ rejected peak จึงยังไม่ยืนยัน margin หรือข้างที่ไม่ผ่าน; ค่า `ANK` เป็น diagnostic transitions ไม่ใช่จำนวน miss | ออกแบบ T-728 เป็น passive evidence ให้เก็บ left/right ankle-rise peak, individual floor และ gate result ของ rejected takeoff ก่อนพิจารณาปรับ detector; ต้องขออนุมัติก่อนแก้ `BasicBounceDetector` และ experiment ภายหลังต้องมี heel raise, knee lift และ standing controls |
