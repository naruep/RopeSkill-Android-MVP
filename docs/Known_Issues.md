...
| KI-018 | 2026-07-28 | Basic Bounce บน APK ปัจจุบันนับขาดในแสงสลัว: T-721 ได้ 20/20, 19/20 และ 17/20 รวม 56/60 | High | Fix awaiting verification | การนับที่รับแล้วมี `AIR=LAND`, `SUP 0` และ trace ไม่มี `LS` หรือ Landing cycle แตกซ้ำ จึงไม่ใช่ regression ของ KI-013; T-722 ที่ commit `20e55fe` ได้แสงปกติ 20/20 ที่ FPS 30.0 และแสงสลัว 19/20 ที่ FPS 25.0 โดย genuine miss เป็น trace `#46 R A0.020 H0.141 ANK` และ V10 `R A0.020/0.023 H0.141/0.157 F7 D240 P39 N42 ANK`; raw ankle peak `0.023` ยังต่ำกว่า rescue floor `0.025`, มี result frames และช่วงก่อน/หลัง peak ครบ จึงไม่ใช่ `N0`, ไม่ใช่ smoothing ลด raw peak ที่ผ่านเกณฑ์ และไม่ใช่การขาด post-peak frames; หลักฐานยืนยันกลไกที่พลาดว่า measured ankle rise ต่ำกว่า floor แต่การทดสอบคู่เดียว ยังแยกไม่ได้ว่ามาจากความสูงการกระโดดจริงหรือ landmark attenuation ในแสงสลัว | T-723 ที่ได้รับอนุมัติลดเฉพาะ Strong-Hip Rescue ankle floor เป็น `0.020` โดยคง threshold, hip gate, bilateral/sync, ratio, smoothing, cooldown และ Landing re-arm guard เดิม; รอ Unit tests/Build และ real-device pilot ก่อนพิจารณาปิด Issue |
