...
Milestone 0–7 ผ่านส่วนหลักบน Samsung Galaxy S23 Ultra แล้ว T-716 ที่ detector baseline commit `591b938` และ Music OFF ได้ Basic Bounce 59/60 หรือ 98.3% แต่ T-717 ที่ commit `7c247eb` ได้แสงปกติ 17/20 และ 15/20 ขณะที่แสงน้อยลงเล็กน้อยได้ 19/20 จึงเปิด KI-013 กลับมาตรวจสอบความแปรผันใกล้ rescue floor; T-718 Passive Cycle Trace V9 ที่ commit `4cf4869` ผ่าน 20/20, ไม่มี suppressed Landing และไม่เปลี่ยน detector behavior แต่ยังไม่ทำให้อาการ T-717 เกิดซ้ำ
...
| KI-013 | 2026-07-25 | Basic Bounce อาจต่ำกว่าเป้าหมายแม้ repeatability รอบก่อนผ่าน | High | Reopened | ลด Strong-Hip Rescue ankle floor จาก `0.030` เป็น `0.025` แล้ว T-715/T-716 ผ่าน แต่ T-717 แสงปกติได้ 17/20 และ 15/20; rejected genuine-looking takeoff มี ankle `0.021–0.024` ต่ำกว่า floor เล็กน้อยแม้ hip `0.125–0.165`; รอบแสงน้อยได้ 19/20 จึงยังระบุแสงเป็นสาเหตุไม่ได้; code review พบ `completedVerticalCycle` อาจจบ AIRBORNE เมื่อเห็นการขึ้นรอบใหม่หนึ่งเฟรม ขณะที่ cooldown กรอง Landing ภายใน 250ms | T-718 ผ่าน 20/20: trace ช่วงกระโดดสลับ `T → LC` ครบ 20 รอบ, `SUP 0`, rescue 14 และมี Landing reason `C` หนึ่งครั้งโดยไม่เพิ่มซ้ำ จึงยืนยันว่า trace ใช้งานได้แต่ยังไม่ reproduce ปัญหา; รัน T-719 แสงปกติ 20 ครั้ง 3 รอบเพื่อจับ `R/LS/C/BC` ตอนเกิด miss ก่อนเสนอ threshold หรือ state-machine change |
