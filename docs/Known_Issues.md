...
| KI-020 | 2026-07-29 | Continuous 100 Jumps ในแสงปกตินับได้ T-726 85/100, T-727 93/100, T-728 83/100 และ T-729 BASE 86/101 ต่ำกว่าเป้าหมาย 95%; รอบที่ไม่มี rope trip ยังเกิดซ้ำ | High | Confirmed / T-730 device attribution pending | T-729 เริ่มและจบใน matched window โดย BASE `J86 A/L86/86 S0 RES81` ตรงกับ App/Result; BIL-only `J86 D+0` ส่วน RES-only `J88 A/L88/88 S0 RES83 D+2`. ผลนี้ตัดเฉพาะ BIL `0.006` และ RES `0.018` แบบลดทีละ gate ออกจากการเป็น fix ที่เพียงพอ แต่ไม่แยก multi-gate failures. T-730 จึงอ่าน BASE rejected evidence ภายนอก detector และรายงาน blocker set จาก exact-paired retained evidence สำหรับ `SYNC/BIL/RATIO/STD-H/RES-A/RES-H`; `RATIO` ใช้ direct multiply parity, peak ordinal ไม่เท่ากับ physical miss, operand ไม่ได้รับรองว่าอยู่ frame เดียวกัน และ unmatched evidence ต้องเป็น `UNATTRIBUTED` | คง BASE `0.010/0.020`; รัน T-730 smoke และ formal 20–30 jumps โดยยืนนิ่งบน `ACTIVE P0` ประมาณ 1 วินาทีก่อนเริ่ม และใช้เฉพาะรอบ `ACTIVE/U0/OV0` ที่ไม่มี tracking reset/rope trip แล้วจับคู่ peak ordinals กับวิดีโอ. หากไม่มี rejected peaks, `U>0`, overflow หรือ actual miss ไม่มี peak event ให้สรุป Inconclusive และออกแบบ evidence ก่อนหน้า peak เพิ่ม โดยยังไม่เปลี่ยน detector |
