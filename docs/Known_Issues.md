...
| KI-020 | 2026-07-29 | Continuous 100 Jumps ในแสงปกตินับได้ T-726 85/100, T-727 93/100, T-728 83/100 และ T-729 BASE 86/101 ต่ำกว่าเป้าหมาย 95%; รอบที่ไม่มี rope trip ยังเกิดซ้ำ | High | Confirmed / T-730 V13 trace pending | Root cause ของ undercount ยังไม่ยืนยัน. T-729 ตัดเฉพาะ BIL `0.006` และ RES `0.018` แบบลดทีละ gate ออกจากการเป็น fix ที่เพียงพอ. T-730 V12 รอบแรกเป็น `INVALID-RESTART/P0`; Smoke-R มี Actual/App 3/3 แต่ global collector เพิ่มถึง pre-jump `P26 C0 R26 U4 OV20` และ final `P52 C3 R49 S0 U5 OV43` เพราะรวม standing landmark jitter. นี่เป็น collector limitation: Counter ไม่มี false count, หลังหยุดไม่เพิ่ม และข้อมูล V12 ไม่พิสูจน์ว่า jitter เป็นสาเหตุของ missed jumps | คง BASE `0.010/0.020` และไม่แก้ `BasicBounceDetector`, threshold หรือ Counter. ยืนยัน Android tests/Build และ V13 smoke ก่อน จากนั้นใช้ timestamped raw trace สูงสุด 512 events และ accepted-cycle bookends แยก `LEAD/WINDOW/BOUNDARY/TAIL`; วิเคราะห์ gate เฉพาะ `WINDOW`. Formal ใช้ counted opening bookend 1 + target 20 + counted closing bookend 1 รวม Actual 22 และยกเลิกรอบเมื่อมี rope trip, auto-pause ก่อน seal/ระหว่าง measurement, explicit invalid reason หรือ trace overflow; ใช้ Screen Recorder จับ sealed frame เพราะ auto-pause ที่ตามมาอาจล้าง overlay |
