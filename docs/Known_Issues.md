...
Milestone 0–7 ผ่านส่วนหลักบน Samsung Galaxy S23 Ultra แล้ว T-716 ที่ detector baseline commit `591b938` และ Music OFF ได้ Basic Bounce 59/60 หรือ 98.3% แต่ T-717 พบความแปรผันด้านความแม่นยำ; T-718 Passive Cycle Trace V9 ผ่าน 20/20 และ T-719 ทำให้อาการเกิดซ้ำในแสงปกติที่ 55/60 พร้อมหลักฐานว่า Strong-Hip Rescue takeoff สามารถอยู่ภายใน Returned-to-Baseline landing band จนเกิด Landing เร็วและวงจรย่อยซ้ำ จึงคง KI-013 เป็น Reopened
...
| KI-013 | 2026-07-25 | Basic Bounce อาจต่ำกว่าเป้าหมายแม้ repeatability รอบก่อนผ่าน | High | Reopened | T-719 ได้ 55/60 โดย AIR/LAND รวม 77/77 และ SUP 22; `LAND − SUP = 55` ตรงกับ Counter แสดงว่าการกระโดดจริง 60 ครั้งถูกแตกเป็นวงจร detector 77 รอบ; trace พบ `LC/LS` เหตุผล `B` และ airborne สั้นผิดปกติถึง 1–145ms พร้อมรูปแบบ `T → LC → T → LS`; สาเหตุหลักคือ Strong-Hip Rescue รับ ankle rise ตั้งแต่ `0.025` แต่ Returned-to-Baseline ใช้ landing band `0.040` ทำให้ rescue takeoff ช่วง `0.025–0.040` เริ่ม AIRBORNE ทั้งที่อยู่ใน landing band และจบ Landing ได้ทันที; Run 2 ยังพบ rejected `A0.023 H0.168` ต่ำกว่า rescue floor เป็นปัจจัยรอง; performance ปกติและ Landing ที่มีปัญหาเป็น `B` ไม่ใช่ `C/BC` จึงไม่สนับสนุนว่าแสงหรือ `completedVerticalCycle` เป็นสาเหตุหลัก | เสนอ T-720 Landing Re-arm Guard: ป้องกัน `B` จนกว่าจะมีหลักฐานการลงหลัง Takeoff โดยคง threshold/rescue floor/cooldown เดิมก่อน เพื่อแยกผลของ state transition; ต้องได้รับข้อตกลงชัดเจนก่อนแก้ `BasicBounceDetector` |
