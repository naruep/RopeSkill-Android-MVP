...
| KI-023 | 2026-07-31 | T-744 static audit พบ diagnostic panels และ totals อาจแสดง/สะสมใน Release | Blocker | Fix implemented / Verification pending | Count-evidence panel และ outer summary condition ไม่ได้ Debug-gate ทั้งก้อน; `TrainingViewModel` ยังสะสม counted evidence, transition counts และ rescue totals ใน Release | T-745 ครอบ UI ด้วย `BuildConfig.DEBUG`, เพิ่ม pure boundary helper/tests และปิด Release payload collectionโดยไม่แตะ detector/Counter; ปิด Issue หลัง Windows verification และ Release APK smoke ผ่าน |
