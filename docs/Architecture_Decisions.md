...
- **Validation:** Pure-Kotlin regression ผ่าน 64/64 รวม `BasicBounceDetectorTest`, `T729ControlledExperimentTest` และ T-730 detector-boundary/direct-ratio/truth-table/lifecycle tests. Android Gradle tests, Build และ Samsung Galaxy S23 Ultra smoke ยังรอยืนยัน เพราะ Codex workspace ดาวน์โหลด Gradle 9.3.0 ไม่ได้
...
- **Revisit when:** Smoke ผ่าน performance/stability gates และ formal 20–30 jumps ที่เริ่มหลังยืนนิ่งบน `ACTIVE P0` ประมาณ 1 วินาที ได้ header `ACTIVE`, `U0`, `OV0`, ไม่มี tracking reset/rope trip, Counter หลังหยุดไม่เพิ่ม, `AIR=LAND`, `SUP 0` และ Result/History ถูกต้อง; ใช้วิดีโอจับคู่ physical misses กับ event IDs ก่อนออกแบบ experiment ถัดไป
