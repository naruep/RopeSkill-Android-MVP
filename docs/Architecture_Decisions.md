...
- **Status:** Accepted for testing / Device smoke conditionally passed; History and formal verification pending
...
- **Validation:** Pure-Kotlin compile/regression ผ่าน 77/77 ครอบคลุม production detector, T-729 parity และ V13 real-detector/timestamp/window/region/capacity/integrity/sealed-exit tests. Device Smoke รอบแรก Actual/App 3/2 ยืนยัน timestamp, accepted anchors, region equations, `OV0` และ BASE isolation แต่ไม่ผ่าน Counter; jump สุดท้ายปรากฏเป็น `#014 T/R` candidate `Q+SH` ใน `TAIL` จึงไม่รวม primary gate totals. Smoke-R2 ได้ Actual/App 3/3, `ALL P54 C3 R51 S0 TR54 OV0`, `SEG L17 W3 B0 T34`, `WIN P3 C3 R0 S0 U0`, gate ทุกตัว 0, BASE `A/L3/3 SUP0 RES1`, หลัง Landing นิ่งประมาณ 13 วินาที, Counter หลังหยุดไม่เพิ่ม และ Result 3/00:22 โดยไม่พบ performance/stability regression. ไม่พบ `SEALED POST-EXIT` แต่ snapshot ก่อนออกไม่เป็น `INVALID`; History ยังไม่ได้เปิด จึงเป็น Conditional Pass. การที่ APK ทำงานบน Samsung Galaxy S23 Ultra ยืนยัน device execution แต่ผล Android `testDebugUnitTest`/`assembleDebug` ยังไม่มี log แยกในหลักฐาน และ Formal 22 jumps ยังไม่ได้รัน
...
- **Revisit when:** หลังยืนยัน History 3/00:22 ให้รัน formal counted-bookend 22-jump protocol ตาม `docs/T730_V13_Formal_22_Jumps.md` และจับ `SEALED POST-EXIT`; หาก formal `WINDOW` ยังรวม standing noise, accepted anchors ไม่เสถียร, trace overflow, event sequence invalid หรือ miss ไม่สร้าง evidence ที่ใช้จัด blocker ได้ ให้แก้ instrumentation ก่อนตีความ KI-020
