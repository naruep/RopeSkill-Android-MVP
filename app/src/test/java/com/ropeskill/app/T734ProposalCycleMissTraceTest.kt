...
class T734ProposalCycleMissTraceTest {
    @Test
    fun matchedAndAirborneUnmatchedPulses_areSeparatedWithoutChangingResult() {
        val collector = T734ProposalCycleMissCollector(enabled = true)
    ...
    fun disabledCollector_neverPublishes() {
        val collector = T734ProposalCycleMissCollector(enabled = false)
    ...
    fun formatter_exposesTraceTotalsAndRecentRows() {
        val text = formatT734ProposalCycleSnapshot(
        ...
        assertTrue(text.contains("T-734 PULSE TRACE V18 MATCHED"))
        assertTrue(text.contains("RAW22 Q22 M21 U1 UA1 T/L21/21 X0 F720"))
        assertTrue(text.contains("#12 +6.250 QU A0.032 H0.118 D132 AIR"))
    }

    private fun T734MotionPulse.marker(): String =
